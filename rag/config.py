...
def load_config(path: str | Path = DEFAULT_CONFIG_PATH) -> dict:
    ...
    # Paths in default.yaml are relative to the repo root (config/ lives one level below it).
    cfg["_project_root"] = str(config_path.resolve().parent.parent)
    return cfg


def resolve_path(cfg: dict, relative_path: str) -> Path:
    """Resolve a path from default.yaml, which are written relative to the repo root."""
